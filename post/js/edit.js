...
function handleFormSubmit(event) {
  ...
  if (!title || !alt || !tag) {
    alert(
      "All fields are required and must not be empty, except for the image URL which must be a valid HTTPS link."
    );
    return;
  }
  ...
  const apiEndpoint = isEditing
    ? `${createAPI}/${username}/${currentEditingId}`
    : `${createAPI}/${username}`;
  ...
  fetch(apiEndpoint, {
    ...
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          "Failed to process post, server responded with an error."
        );
      }
    ...
    .catch((error) => {
      ...
      alert(
        `Failed to ${isEditing ? "update" : "create"} post: ${error.message}`
      );
    });
}
...
function updateCharacterCount() {
  ...
  const countFeedback =
    document.getElementById("char-count-feedback") ||
...
function displayOldPosts(posts) {
  ...
  posts.forEach((post) => {
    ...
    editBtn.setAttribute("aria-label", "Edit " + post.title);
    editBtn.onclick = () => loadPostInForm(post);
    postDiv.appendChild(editBtn);

    const deleteBtn = document.createElement("button");
    deleteBtn.classList.add("old-post-delete");
    deleteBtn.setAttribute("aria-label", "Delete " + post.title);
